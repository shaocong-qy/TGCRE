...
from model import TGCRE
from transformers import get_linear_schedule_with_warmup
from argparse import ArgumentParser

parser = ArgumentParser()
parser.add_argument("--seed", type=int, default=42)
parser.add_argument("--gamma", type=float, default=0.03, help="Loss function: margin factor gamma")
...
parser.add_argument("--unseen", type=int, default=5, help='Number of unseen class')
...
parser.add_argument("--rel_split_seed", type=str, default='41ori')
...
parser.add_argument("--pretrained_model", type=str, default='../bert-base', help='huggingface pretrained model')
...
model = TGCRE.from_pretrained(args.pretrained_model, config=config)
model = model.cuda()
...
for epoch in range(args.epochs):
    ...
    running_loss = 0.0
    match_loss = 0.0  # q和grad match_score
    ...
    for step, data in enumerate(trainloader):
        ...
        outputs, out_sentence_emb, e1_h, e2_h, _, _ = model(input_ids=tokens_tensors,
